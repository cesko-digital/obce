...
export async function addExtraData(
  subjects: Subjekt[],
  sources: DataSource<Object>[]
): Promise<Subjekt[]> {
  var out: Subjekt[] = [];
...
async function addExtraDataToSubject<T>(
  subject: Subjekt,
  ...
  if (cachedValue != null) {
    ...
    console.debug(
      `Cache miss for “${key}” of subject ${label}, will try to get fresh data.`
    );
    // TODO: Retry?
